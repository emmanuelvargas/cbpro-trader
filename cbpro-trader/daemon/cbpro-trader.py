...
class CBProTrader(object):
    ...
    def init_engine_and_indicators(self):
        ...
        auth_client = cbpro.AuthenticatedClient(
            self.config['key'], self.config['secret'],
            self.config['passphrase'], api_url=api_url)
        ...
        for cur_period in self.config['periods']:
            ...
            if cur_period.get('meta'):
                new_period = period.MetaPeriod(
                    ...
                    fiat=fiat_currency, product=cur_period['product'],
                    name=cur_period['name'], cbpro_client=auth_client)
            else:
    ...
    def run(self):
        ...
        self.interface.update(
            self.trade_engine, self.indicator_subsys.current_indicators,
